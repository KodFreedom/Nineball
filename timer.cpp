...
#define TIMER_POS_X (340.0f)
#define TIMER_POS_Y (55.0f)
#define TIMER_WIDTH (40.0f)
#define TIMER_HEIGHT (54.0f)
#define TIMER_LENGTH (0.0f)//数字間の長さ
...
HRESULT MakeVerTexTimer(LPDIRECT3DDEVICE9 pDevice)
{
	...
	{
		//頂点座標の設定（2D座標、右回り）
		pVtx[nCnt * 4].pos = D3DXVECTOR3(TIMER_POS_X + TIMER_WIDTH * nCnt, TIMER_POS_Y, 0.0f);
		pVtx[nCnt * 4 + 1].pos = D3DXVECTOR3(TIMER_POS_X + TIMER_WIDTH * nCnt + TIMER_WIDTH, TIMER_POS_Y, 0.0f);
		pVtx[nCnt * 4 + 2].pos = D3DXVECTOR3(TIMER_POS_X + TIMER_WIDTH * nCnt, TIMER_POS_Y + TIMER_HEIGHT, 0.0f);
		pVtx[nCnt * 4 + 3].pos = D3DXVECTOR3(TIMER_POS_X + TIMER_WIDTH * nCnt + TIMER_WIDTH, TIMER_POS_Y + TIMER_HEIGHT, 0.0f);
		...
		//頂点カラーの設定(0～255の整数値)
		pVtx[nCnt * 4].color = D3DCOLOR_RGBA(112, 52, 10, 255);
		pVtx[nCnt * 4 + 1].color = D3DCOLOR_RGBA(112, 52, 10, 255);
		pVtx[nCnt * 4 + 2].color = D3DCOLOR_RGBA(112, 52, 10, 255);
		pVtx[nCnt * 4 + 3].color = D3DCOLOR_RGBA(112, 52, 10, 255);

		//texture頂点
		pVtx[nCnt * 4].tex = D3DXVECTOR2(0.0f, 0.0f);
		pVtx[nCnt * 4 + 1].tex = D3DXVECTOR2(0.1f, 0.0f);
		pVtx[nCnt * 4 + 2].tex = D3DXVECTOR2(0.0f, 1.0f);
		pVtx[nCnt * 4 + 3].tex = D3DXVECTOR2(0.1f, 1.0f);
	}
